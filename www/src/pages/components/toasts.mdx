...
A Toast can also automatically hide after X milliseconds using the `autohide` prop in combination with the `delay` prop to sepecify the delay. But be aware that it will only trigger the `onClose` function, you have to manually set the show property.
